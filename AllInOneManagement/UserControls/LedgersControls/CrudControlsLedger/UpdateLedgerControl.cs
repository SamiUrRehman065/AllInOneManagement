using Microsoft.Data.SqlClient;
using System.Data;

namespace FactoryManagementSystem.UserControls.LedgersControls.CrudControlsLedger
{
    public partial class UpdateLedgerControl : UserControl
    {
        int Id;
        decimal? Amount;
        string? CName;
        public UpdateLedgerControl(int id, decimal? amount, string? CustomerName)
        {
            InitializeComponent();
            Amount = amount;
            CName = CustomerName;
            Id = id;
            txtCName.Text = CName;
            DatePicker.Value = DateTime.Now;
            if (Amount < 0)
            {
                ClearLeadgweCheckBox.Visible = false;
            }

        }

        private void ClearLeadgweCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ClearLeadgweCheckBox.Checked)
            {
                txtAmount.Text = Amount.ToString();
                txtCName.Text = CName;
                txtAmount.ReadOnly = true;
            }
            else
            {
                txtAmount.Text = string.Empty;
                txtCName.Text = CName;
                txtAmount.ReadOnly = false;
            }
        }

        private void txtAmount_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!Char.IsDigit(e.KeyChar) && e.KeyChar != 8 && e.KeyChar != '.')
            {
                e.Handled = true; // Cancel the input if it's not a digit, decimal point, or Backspace
            }
            if (e.KeyChar == '.' && txtAmount.Text.Contains('.'))
            {
                e.Handled = true; // Cancel the input if there is already a decimal point
            }
        }

        private async void btnClear_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtAmount.Text) || string.IsNullOrWhiteSpace(txtDescription.Text))
            {
                MessageBox.Show("Please enter a valid amount or description to clear the ledger", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
            {
                await ClearLedger(Id, txtDescription.Text, DatePicker.Value, decimal.Parse(txtAmount.Text));
            }
        }

        private async Task ClearLedger(int LedgerId, string? description, DateTime Date, decimal? Amount = null)
        {

            try
            {
                using (SqlConnection conn = new(DatabaseOperation.GetConnectionString()))
                {
                    await conn.OpenAsync();
                    using (SqlCommand cmd = new("ClearLedger", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@LedgerID", LedgerId);
                        cmd.Parameters.AddWithValue("@Description", description);
                        cmd.Parameters.AddWithValue("@DateNow", Date);
                        // Handle null Amount
                        if (Amount.HasValue)
                            cmd.Parameters.AddWithValue("@Amount", Amount.Value);
                        else
                            cmd.Parameters.AddWithValue("@Amount", DBNull.Value); // Pass NULL to SQL

                        int result = await cmd.ExecuteNonQueryAsync();
                        if (result > 0)
                        {
                            MessageBox.Show("Ledger cleared successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);

                        }
                        else
                        {
                            MessageBox.Show("Error In Clearing Ledger. Plz Check Your Input!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error clearing ledger: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
